...
import 'package:allinonecalc/utils/my_print.dart';
...
class DatePresentation {
  ...
  static String yMMMdFormatter(Timestamp timeStamp) {
  ...
  static int getDifferenceBetweenDatesInDays(DateTime startDateTime, DateTime endDateTime, {bool canNegative = false}) {
    DateTime new1 = DateTime(startDateTime.year, startDateTime.month, startDateTime.day);
    DateTime new2 = DateTime(endDateTime.year, endDateTime.month, endDateTime.day);

    //MyPrint.printOnConsole("Start Date:${startDateTime.day}, Month:${startDateTime.month}, Year:${startDateTime.year}");
    //MyPrint.printOnConsole("End Date:${endDateTime.day}, Month:${endDateTime.month}, Year:${endDateTime.year}");

    Duration duration = new2.difference(new1);
    if(duration.inDays.isNegative) {
      if(canNegative) return duration.inDays;
      else return duration.inDays * -1;
    }
  ...
  static String ddMMMMyyyyFormatter(Timestamp date){
    String formattedDate = DateFormat('EEE dd, MMMM yy ').format(date.toDate());
    return formattedDate;
  }
